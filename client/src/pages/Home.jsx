...
const FeatureCard = ({ icon: Icon, title, desc, delay = 0 }) => (
    <motion.div
        ...
        viewport={{ once: true }}
        transition={{ delay }}
        className="group relative bg-secondary/20 border border-border/50 rounded-2xl p-6 md:p-8 backdrop-blur-xl hover:border-primary/40 hover:-translate-y-2 transition-all duration-500 overflow-hidden"
    >
...
const Home = () => {
    ...
    return (
        <div className="min-h-screen bg-background text-foreground overflow-x-hidden selection:bg-primary/30 selection:text-primary">
            ...
            <section className="relative pt-24 sm:pt-32 lg:pt-48 pb-20 px-4 sm:px-6">
                ...
                >
                    <motion.div variants={fadeInUp} className="flex justify-center">
                        <div className="inline-flex items-center gap-3 px-4 py-1.5 rounded-full bg-secondary/50 border border-border/50 backdrop-blur-md">
                    ...
                    <motion.p
                        variants={fadeInUp}
                        className="max-w-2xl mx-auto text-muted-foreground text-[clamp(1rem,2.8vw,1.35rem)] font-light leading-relaxed px-4"
                    >
                    ...
                    <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row gap-4 justify-center pt-8">
                        {isAuthenticated ? (
                            <Link to="/dashboard" className="group h-14 px-10 rounded-2xl bg-primary text-primary-foreground flex items-center justify-center gap-3 font-bold text-[16px] shadow-[0_0_40px_-10px_var(--primary)] hover:scale-105 transition-all">
                                Go to Dashboard <ArrowRight size={20} />
                            </Link>
                        ) : (
                            <>
                                <Link to="/register" className="h-14 px-10 rounded-2xl bg-foreground text-background font-bold text-[16px] flex items-center justify-center hover:scale-105 transition-all">
                                    Get Started
                                </Link>
                                <Link to="/notes" className="h-14 px-10 rounded-2xl bg-secondary/50 border border-border backdrop-blur-xl hover:bg-secondary font-bold text-[16px] flex items-center justify-center transition-all">
                                    Browse as Guest
                                </Link>
                            </>
                        )}
                    ...
                    >
                        ...
                        <div className="rounded-3xl bg-card/80 border border-border shadow-2xl overflow-hidden backdrop-blur-3xl aspect-[16/10] md:aspect-video text-left">
                            <div className="h-10 md:h-12 bg-secondary/50 border-b border-border flex items-center justify-between px-4 md:px-6">
                                <div className="flex gap-2">
                                    <span className="w-2.5 h-2.5 bg-destructive/80 rounded-full" />
                            ...
                            <div className="grid grid-cols-12 h-full">
                                <div className="hidden md:block col-span-3 border-r border-border p-6 space-y-4 text-left">
                                    {[
                                        ...
                                        <div key={i} className={`flex items-center gap-3 text-[11px] font-bold ${i === 0 ? 'text-primary' : 'text-muted-foreground'}`}>
                                ...
                                <div className="col-span-12 md:col-span-9 p-6 md:p-10 font-mono text-[12px] md:text-[15px] text-foreground/80 space-y-4 overflow-hidden text-left bg-background/50">
                                    <div className="flex items-center gap-4 text-green-400">
                                        <div className="w-2 h-2 rounded-full bg-green-400 shadow-[0_0_10px_rgba(52,211,153,0.8)]" />
                                        <span># SYSTEM: CONNECTED</span>
                                    </div>
                                    <div className="space-y-1">
                                        <p className="text-muted-foreground italic">// Starting your session...</p>
            ...
            {/* Features Section */}
            <section className="py-32 px-4 sm:px-6 relative bg-background/50">
            ...
            {/* Footer */}
            <footer className="relative pt-20 pb-12 px-4 sm:px-6 border-t border-border bg-background">
